    ...
    QDialog( parent )
{
    ...
    addressTextBox->setText(DEFAULT_ADDRES);
    ...
    port = addressTextBox->text().replace( QRegExp(".*:"), "" ).toInt();
    login = loginTextBox->text();
    pass = passTextBox->text();
    address = addressTextBox->text().replace(QRegExp(":.*"), "");
...
QWidget* ConnectionInfoDialog::getParent()
{
    return mainWindow;
}
